...
export const routes: Routes = [
  ...
  { path: 'sistema/login', loadComponent: () => import('./features/login/login').then((m) => m.Login) },
  {
    ...
    children: [
      ...
      { path: 'atencion/:encounterId/odontograma', canActivate: [permissionGuard], data: { title: 'Odontograma', permission: 'CLINICA_LEER' }, loadComponent: () => import('./features/odontogram/pages/odontogram-editor/odontogram-editor').then((m) => m.OdontogramEditor) },
      ...
      { path: 'atencion', canActivate: [permissionGuard], data: { title: 'Atención clínica', permission: 'CLINICA_LEER' }, loadComponent: () => import('./features/clinical/pages/clinical-workspace/clinical-workspace').then((m) => m.ClinicalWorkspace) },
      ...
      { path: 'copiloto', canActivate: [permissionGuard], data: { title: 'Copiloto IA', permission: 'IA_LEER' }, loadComponent: () => import('./features/copilot/pages/copilot-workspace/copilot-workspace').then((m) => m.CopilotWorkspace) },
      {
        path: 'pacientes',
