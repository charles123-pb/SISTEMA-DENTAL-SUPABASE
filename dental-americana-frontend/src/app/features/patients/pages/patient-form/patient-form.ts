...
export class PatientForm implements OnInit {
  ...
  submit(force = false): void {
    ...
    if (!id && !force) {
      ...
      this.api.duplicates(value.documentNumber, value.mobile, value.birthDate, value.paternalSurname).subscribe({
        next: (items) => {
          this.loading.set(false); this.duplicates.set(items);
          if (items.length) this.duplicateReviewOpen.set(true); else this.persist();
        },
        error: () => { this.loading.set(false); this.error.set('No se pudo comprobar si el paciente ya existe.'); },
      });
      return;
    }
  ...
  private persist(): void {
    ...
    request.subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        ...
        this.error.set(error.error?.message || (error.status === 409 ? 'El paciente fue modificado o ya existe.' : 'No se pudo guardar la ficha.'));
      },
    });
  }
  ...
  private load(id: number): void {
    ...
    this.api.get(id).subscribe({
      ...
      error: () => { this.loadingPatient.set(false); this.error.set('No se pudo cargar el paciente.'); },
    });
  }
}
