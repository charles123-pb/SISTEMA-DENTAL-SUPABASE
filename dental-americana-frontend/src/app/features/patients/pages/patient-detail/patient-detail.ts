...
export class PatientDetailPage implements OnInit {
  ...
  download(file: PatientFile): void {
    ...
    this.api.downloadFile(patient.id, file.id).subscribe({
      next: (response: HttpResponse<Blob>) => {
        const blob = response.body;
        if (blob) {
          const url = URL.createObjectURL(blob); const anchor = document.createElement('a');
          anchor.href = url; anchor.download = file.originalName; anchor.click(); URL.revokeObjectURL(url);
        }
        this.downloadingId.set(null);
      },
      error: () => { this.downloadingId.set(null); this.error.set('No se pudo descargar el archivo.'); },
    });
  }
  ...
  changeStatus(): void {
    ...
    this.api.changeStatus(p.id, !p.active, p.version).subscribe({
      ...
      error: (error: HttpErrorResponse) => this.handleError(error, 'No se pudo cambiar el estado del paciente.'),
    });
  }
  ...
  private load(id: number): void {
    ...
    this.api.get(id).subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        ...
        this.error.set(error.status === 404 ? 'El paciente no existe o ya no está disponible.' : 'No se pudo cargar la ficha del paciente.');
      },
    });
  }

  private run<T>(request: { subscribe(observer: { next(value: T): void; error(error: HttpErrorResponse): void }): unknown }, message: string, reset: () => void): void {
    this.saving.set(true); this.clearMessages();
    request.subscribe({
  ...
  private handleError(error: HttpErrorResponse, fallback: string): void {
    this.saving.set(false); this.error.set(error.error?.message || fallback);
  }
